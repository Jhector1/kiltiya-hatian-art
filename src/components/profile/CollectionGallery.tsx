    ...
    <div className="p-4">
      {/* ...header & tabs unchanged */}
       <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
        <h3 className="text-2xl font-bold text-gray-800 mb-4 md:mb-0">Your Purchased Art</h3>
          ...
          {TABS.map((tab) => (
            <button
              ...
              className={[
                ...
                filter === tab ? "bg-indigo-600 text-white shadow-lg" : "bg-gray-200 text-gray-700 hover:bg-gray-300",
              ].join(" ")}
            >
    ...
    <div className="relative z-20" onClick={(e) => e.stopPropagation()}>
      <button
        className={`text-xs px-3 py-1.5 rounded-full shadow ${
          anyBusy ? "bg-gray-300 text-gray-700 cursor-wait" : "bg-indigo-600 text-white hover:bg-indigo-700"
        }`}
        aria-haspopup="menu"
        aria-expanded={open}
        onClick={() => setOpen((v) => !v)}
      ...
      {open && (
        <div
          ...
          onMouseLeave={() => { if (!anyBusy) setOpen(false); }}
        >
