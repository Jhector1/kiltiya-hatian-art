...
export async function POST(req: NextRequest,   { params }: { params: Promise<{ designId: string }> }
  ...
  try {
    ...
    const { dataUrl, width = 800, quality = 70, overwritePreview = true } = (await req.json()) as Body;

    if (typeof dataUrl !== "string" || !dataUrl.startsWith("data:image/")) {
      return NextResponse.json({ error: "Expected dataUrl (data:image/...)" }, { status: 400 });
    }
    ...
    if (!base64) return NextResponse.json({ error: "Malformed dataUrl" }, { status: 400 });

    const input = Buffer.from(base64, "base64");

    const webp = await sharp(input)
      .resize({ width: safeWidth, withoutEnlargement: true, fit: "inside", background: { r: 255, g: 255, b: 255, alpha: 0 } })
      ...
      const stream = cloudinary.uploader.upload_stream(
        {
          ...
          format: "webp",
          invalidate: true, // make sure clients don’t see a cached old preview
        },
    ...
    return NextResponse.json({ ok: true, previewUrl, publicId: upload.public_id });
  } catch (err: any) {
